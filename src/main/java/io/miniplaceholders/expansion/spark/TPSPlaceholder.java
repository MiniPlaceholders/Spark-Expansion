...
public final class TPSPlaceholder implements GlobalTagResolver {

    @Override
    public Tag tag(ArgumentQueue argumentQueue, Context context) {
        final DoubleStatistic<StatisticWindow.TicksPerSecond> statistics;
        try {
            statistics = SparkProvider.get().tps();
        } catch (IllegalStateException e) {
            // Spark not loaded yet
            return null;
        }
